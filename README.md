...
| # | 프롬프트 | 산출물 |
| --- | --- | --- |
| 1 | 기업 분석 리포트 만들기 | 채팅 — 이후 모든 단계의 재료 |
| 2 | ① 이력서 원고 다시 쓰기 | 채팅 — 순서·표현을 고친 원고 + Before/After 표 |
| 3 | ② 자기소개서 원고 다시 쓰기 | 채팅 — 소재 배분표 + 문항별 소제목·본문·글자 수 |
| 4 | ③ 이력서 + 자기소개서 통합 HTML | **HTML 파일** — 앞의 두 원고를 한 파일로 통합 |
| 5 | 1분 자기소개 + 매칭표 HTML | **HTML 파일** — 매칭표 + 55~70초 스크립트 + 캐치프레이즈 |
| 6 | 핵심 경험 STAR 답변 HTML | **HTML 파일** — 질문 4개 + STAR 에피소드 |
| 7 | 예상 질문 10개 + 압박 방어 논리 HTML | **HTML 파일** — 질문 10개 표 + 압박질문 5개 + 역질문 3개 |

### 2단계는 왜 3개로 나뉘어 있는가

이력서와 자기소개서를 각각 HTML로 받으면 **사용자가 두 파일을 직접 합쳐야 합니다.**
그래서 ①②는 채팅으로 원고만 만들고, ③에서 **같은 대화창의 앞 결과를 그대로 받아**
하나의 파일로 통합합니다. 사용자가 다시 붙여넣을 필요가 없습니다.

②③ 카드에는 "같은 대화창에서 이어서" 배지가 붙고, 프롬프트 첫 줄에도 경고가 들어갑니다.

### 산출물이 왜 HTML인가

2~6번 프롬프트는 **조언이 아니라 완성된 문서**를 만듭니다.
수업 교안(`01_홍요셉_이력서_자소서_포트폴리오_v4.html`)과 **동일한 디자인 시스템**
(A4 210×297mm, Pretendard, `.page` / `.sec-h` / `.qa-body` 등 같은 클래스명)을
프롬프트에 내장했기 때문에, 받은 코드를 그대로 저장하면 수업 결과물과 같은 형식의 파일이 열립니다.

저장 방법: 코드 블록 복사 → 메모장 붙여넣기 → `.html`로 저장(UTF-8) → 더블클릭
